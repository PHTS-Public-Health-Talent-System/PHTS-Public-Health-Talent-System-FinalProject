...
export default function HeadHRPayrollPage() {
  ...
  const pendingRows = useMemo(() => rows.filter((row) => row.status === 'WAITING_HR'), [rows]);
  const processedRows = useMemo(
    () =>
      rows.filter(
        (row) =>
          row.status === 'WAITING_HEAD_FINANCE' || row.status === 'WAITING_DIRECTOR' || row.status === 'CLOSED',
      ),
    [rows],
  );
    ...
    <div className="p-8 space-y-6 pb-20">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">ตรวจสอบและอนุมัติรอบจ่ายเงิน พ.ต.ส. ในขั้นทรัพยากรบุคคล</p>
        </div>
        <div />
      </div>
            ...
            <Table>
              ...
              <TableBody>
                {isLoading ? (
                  ...
                  <TableRow>
                    <TableCell colSpan={7} className="h-24 text-center text-muted-foreground">
                      ไม่มีรอบที่รอหัวหน้า HR อนุมัติ
                    </TableCell>
                  </TableRow>
                ) : (
                  pendingRows.map((row) => (
                    <TableRow key={row.periodId} className="group hover:bg-muted/30 border-border">
                      <TableCell className="font-mono text-sm">
                        <Link
                          href={`/head-hr/payroll/${row.periodId}`}
                      ...
                      <TableCell>
                        <div className="flex items-center justify-end gap-2">
                          <TableRowViewAction href={`/head-hr/payroll/${row.periodId}`} />
                          <Button
                            variant="outline"
                            size="icon"
