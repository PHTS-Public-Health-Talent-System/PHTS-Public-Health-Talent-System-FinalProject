...
export async function getAuditSummary(params?: ApiParams) {
  const res = await api.get<ApiResponse<ApiPayload>>("/audit/summary", {
    params,
  });
  return res.data.data;
}
...
export async function searchAuditEvents(params?: ApiParams) {
  const res = await api.get<ApiResponse<ApiPayload>>("/audit/events", {
    params,
  });
  return res.data.data;
}
...
export async function exportAuditEvents(params?: ApiParams) {
  const res = await api.get<ApiResponse<ApiPayload>>("/audit/export", {
    params,
  });
  return res.data.data;
}
