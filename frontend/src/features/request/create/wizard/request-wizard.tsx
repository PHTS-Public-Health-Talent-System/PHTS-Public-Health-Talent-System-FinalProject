...
export function RequestWizard({ initialRequest, returnPath, prefillUserId }: RequestWizardProps) {
  const [currentStep, setCurrentStep] = useState(1);
  const requiresSignature = !prefillUserId;
  ...
  if (requiresSignature && !hasSignature) missingReasons.push('ลายเซ็น');
                ...
                <TooltipTrigger asChild>
                  <span tabIndex={0}>
                    <Button
                      ...
                      variant="success"
                      className="min-w-[150px] gap-2 shadow-md"
                    >
                      {isSubmitting ? (
